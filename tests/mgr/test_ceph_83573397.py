...
CREATE_OSD_CRUSH_RULE = "ceph osd crush rule create-simple {RULE_NAME} default rack firstn"
CREATE_OSD_POOL_WITH_RULE = "ceph osd pool create {POOL_RACK_NAME} 64 64 replicated {RULE_NAME}"
...
def run(ceph_cluster, **kw):
    ...
    script = __script.format(MGR_NODE=mgr_node.hostname,
                             MGR_RESTFUL_PORT=config.get('mgr_restful_port'),
                             RELATIVE_URI=config.get('relative_uri'),
                             USER=cred['user'],
                             PASSWORD=cred['password'])

    script_file = ceph_installer.write_file(sudo=True, file_name=file_name, file_mode="w")
    script_file.write(script)
    script_file.flush()

    out, err = ceph_installer.exec_command(cmd="python {SCRIPT_FILE}".format(SCRIPT_FILE=file_name))
    out, err = out.read().decode().strip(), err.read().decode().strip()
